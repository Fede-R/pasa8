...
function submitForm() {
  ...
  // Ajax post
  $.ajax({ type: 'POST', url: '/api/preferencesmaes', data: formData,
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="enfermedad-card ui `+cardColors[index]+` card" style="display:none" onclick="clickCard(`+i+`)">
        <div class="image">
          <img src="/static/images/dishesmaes/`+data[i].id+`_1.jpeg">
        </div>
        <div class="content">
  ...
  for (i = 1; i <= currentData[id].images; i++) {
    if (i == 1) {
      ...
      <div id="tab`+(i-1)+`" class="ui bottom attached tab active" data-tab="`+(i-1)+`">
        <img src="/static/images/dishesmaes/`+currentData[id].id+`_`+i+`.jpeg" style="width:100%"/>
      </div>`;
    } else {
      imagesText += `
